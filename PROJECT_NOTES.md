...
현재 디자인은 베이지/크림 톤(`--bg-main #F4F0E8`)의 차분한 시스템이며, 글자는 진한 먹색(`--text-main #171512`),
포인트는 블루그레이(`--accent-main #2F4658`)와 코퍼 브라운(`--accent-warm #A66A43`) 입니다. Anthropic 의 정돈된 톤에서 영감을 받았지만 색·문구·구성은 루네르비아만의 것으로 새로 정리했습니다.
...
## 2026-05-19 베이지 톤 리메이크 — 4-뷰 SPA 로 단순화

- Anthropic 에서 영감을 받은 **베이지/크림 톤**으로 사이트 전체를 새로 만들었습니다. 색·구성은 루네르비아만의 것으로 정리.
- 사이트 구조를 **4-뷰 SPA** 로 단순화했습니다 (한 파일 안에서 섹션 전환):
  - **Home** — hero + 인트로 타이핑
  - **Products** — 제품 3장 카드
  - **Why LUNERVIA** — 본문 + 핵심 가치 3개 (Simplicity / Trust / Human-centered Software)
  - **Contact** — 이름·이메일·문의 유형·메시지 폼 (백엔드 자리만 분리)
- 상단 메뉴는 `LUNERVIA / Products / Why LUNERVIA / Contact` 4개로 축소.
- **전체화면 코드 인트로 오버레이를 제거**하고, 그 타이핑 모션(`안녕하세요 ><`)을 hero 상단 `.hero-intro` 칩 안으로 옮겼습니다. 사이트는 곧장 사용 가능.
- 뷰 전환 모션: opacity + translateY + scale + blur, 460ms `cubic-bezier(0.22,0.68,0.24,1)`.
- 색상은 모두 `:root` 변수로 통일 (`--bg-main`, `--text-main`, `--accent-main`, `--accent-warm` …). 하드코딩 색은 가능한 한 제거.
- 모바일에선 햄버거 메뉴 (열림 시 X 변형) + 카드/값 1열 자동 전환.
- 푸터 브랜드 텍스트를 키워서 (`clamp(1.6rem, 3.2vw, 2.4rem)`) 브랜드 풋프린트로 작동하게 함.
- 로고는 텍스트 워드마크 `LUNERVIA` 사용. 이미지 로고를 나중에 끼우려면 `.brand-text` 자리에 `.brand-img` 를 넣으면 됨.
- 기존 서브 페이지(`projects.html`, `proof-board.html`, `reason.html`, `study-room.html`, `updates.html`)는 **메인의 해당 뷰로 리다이렉트**하는 작은 파일로 교체. 옛 URL 공유분이 깨지지 않도록 함.
