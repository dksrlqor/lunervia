...
## 2026-07-15 (5) — 달토끼 제거 (사용자 지시)

사용자 "그냥 토끼는 없애줘" → 순환에서 달토끼 전체 삭제. 순환 = **초승달 → 고리 행성 → 향유고래**(3형태). RABBIT_PARTS/PIVOTS/REACH/rabbitTargets·bakeLimbs 토끼 분기 삭제, SHAPE_KIND 3개, arts=[null,null,whale]. 지구조(shapeIdx===0)·prevShape 로직 무영향. tsc 0·eslint 0·build 10라우트·fresh 탭 콘솔 0. 토끼 좌표가 다시 필요하면 이 커밋의 부모(`e8375cd`) 참조.

## 2026-07-15 (4) — 실루엣 v2(레퍼런스 기반) + 입자 섬세화 + 히어로 엔진 강건화 + 전체 버그 스윕

사용자: 레퍼런스 이미지 2장(떡방아 찧는 달토끼 일러스트, 향유고래) 제공 — "섬세하게 똑바로 많이, 별을 더 섬세하게, 전체 버그 점검, 기술적으로 훨씬 튼튼하게".

- **실루엣 v2** (`MoonParticles.tsx`): ASCII 시뮬로 튜닝 후 이식. **달토끼 = 떡방아 포즈**(서서 절굿공이를 든 옆모습: 머리·몸통·귀2·팔·절굿공이 자루+머리·뒷다리·발·꼬리·절구 테+몸, 눈 1) / **향유고래 = 대각선 상승**(뭉툭한 대두+앞머리 블록·좁은 아래턱·등혹+너클2·두 갈래 꼬리·가슴지느러미, 눈 1). 부위(el)마다 관절 그룹 g 를 달아 **샘플 시점에 그룹·가중치(피벗 거리/reach)를 굽는다**(ArtMap{G,W,P}) — 런타임 영역 판정 제거로 부위 경계 어긋남 원천 차단.
- **관절 v2**: 그룹별 각을 프레임당 한 번만 trig(bakeLimbs, from/to 두 벌) → 입자별로는 피벗 기준 **정확 회전을 w 로 보간**(뿌리 고정·끝이 휨, 입자당 trig 0·할당 0). 토끼 = 절굿공이 쿵덕(0.13rad, g1=팔+자루+공이머리) + 귀 쫑긋(0.06rad 좌우 위상차), 고래 = 후미+꼬리 살랑(0.1rad). 모프 중 출발(1-e)/목표(e) 블렌드 유지.
- **섬세화**: 성단 입자 1100→**1500**(모바일 640→820), 개별 r 0.9배(더 가는 입자). 딥필드 밀도 /6500→/5400(상한 300→360). 외곽선 대역 더 얇게(0.92~1.0 샘플·0.9 수축 기각).
- **강건화(엔진)**: ① **적응 품질** — 이동평균 프레임 26ms 초과 시 밀도 1회 자동 하향(ps/stars 뒤에서 잘라 targets/arts 인덱스 정렬 유지, glint 무효화, 재초기화에도 유지) ② **DPR 변화 감지**(resolution 미디어쿼리 체인 — 모니터 이동·줌에서 백킹스토어 재생성) ③ **contextlost/restored** 복구 ④ glint 인덱스 가드.
- **강건화(Hero)**: 프레임당 getBoundingClientRect 제거 — **트랙 위치·travel·vh 캐시**(RO+resize 에서만 재측정), **무변화 시 DOM 쓰기 0**(p/v 엡실론 게이트, 최상단 대기 중 쓰기 없음), 시차 래퍼·수평선에 will-change-transform(6 레이어).
- **버그 스윕**: console.log/debugger/any 잔재 0(grep), tsc 0·eslint 0·build 10라우트, fresh 탭에서 홈→/work→/why→404 누적 콘솔 0, 320/375 오버플로 0, sticky 핀 실측(0/320/647 고정·900 릴리스), Coena(캔버스 0·구이·카드 3) 회귀 없음. 실루엣 가독은 ASCII 래스터(1500점, 폴백 0)로 확인.
- ⚠ 재확인된 함정: **PS5.1 Get-Content/-replace 로 UTF-8 파일 왕복 금지**(한글 파일은 Write 도구로만) / 콘솔 검증은 반드시 새 탭(seed 버퍼는 과거 HMR 메시지 잔류).

## 2026-07-15 (3) — 성단 순환에 달토끼·우주 고래 추가, 큰 별 형태 제거

사용자: "달·별·행성 말고 귀여운 동물 같은 거 없냐" → 4후보 제안(AskUserQuestion) → **달토끼+고래 선택** + 지시 3: ① 기존 큰 별(별만 그리는 형태) 순환에서 제거 ② 동물 2회 연속 금지 ③ 동물은 조금씩 움직이게.

- **순환 교체**: `crescent → rabbit → planet → whale`(4형태, 동물 사이를 천체가 가른다). `starTargets` 삭제(`starPoint` 는 초승달의 로고 별이 써서 유지). 지구조는 shapeIdx===0 키라 무영향.
- **실루엣 시스템**(`MoonParticles.tsx`): 형태 = **회전 타원 합집합**(`El=[cx,cy,rx,ry,rot]`, R 배수). 대역 재사용 — 채움(B1, 바운딩박스 rejection)/외곽선(B1~B2, **임의 타원 경계 파라메트릭 샘플 + 0.88 수축 합집합 내부면 기각** — 진짜 테두리만 남고, 타원 균등 선택이라 귀·지느러미 같은 작은 부위가 더 촘촘=가독성↑)/강조(B2~, 45% 눈 클러스터 r0.045R + 55% 윤곽). 달토끼 = 머리+몸통+긴 귀 2(±0.18rad)+앞발 2+눈 2점(정면). 고래 = 몸통+꼬리자루+두 갈래 꼬리+가슴지느러미+눈(왼쪽=카피 쪽을 봄, 최대 x 1.05R 로 우측 블리드 억제).
- **관절 모션**: `artOffset(kind,x,y,now)` — 결과는 모듈 let aox/aoy(프레임당 튜플 할당 0). 토끼 = 귀 영역(-y/R>0.25)만 귀뿌리(±0.17R,-0.2R) 피벗 회전 0.09rad·좌우 위상 다름(쫑긋), 고래 = 꼬리(x/R>0.3) 피벗(0.3R,0) 회전 0.1rad(살랑). hold 에선 그대로, **morph 중엔 출발(1-e)·목표(e) 가중 블렌드**(prevShape 추적). reduced-motion 미적용. 시뮬 실측: 귀 끝 0.067R·꼬리 끝 0.079R(데스크톱 ≈14~17px), NaN 0.
- **검증**: tsc 0·eslint 0·build 10라우트·fresh 탭 콘솔 0·오버플로 0. 실루엣은 **Node ASCII 래스터(1100점)**로 가독 확인 — 토끼(귀2·눈2·발) / 고래(몸통·꼬리 V·지느러미·눈) 모두 또렷, rejection 폴백 0/1100. ⚠ 프리뷰 seed 탭의 read_console 은 과거 HMR 버퍼를 계속 반환 — **콘솔 검증은 반드시 새 탭에서**(fresh 0 재실증).
